...
void receive_ack_from_workers(int msqid, int num_workers) {
    ...
    while(count < num_workers){
        if(msgrcv(msqid, &buf, MAX_MSG, BROADCAST_MTYPE, 0) == -1){
            perror("msgecv failed");
            exit(EXIT_FAILURE);
        }
        if(buf.mtype == BROADCAST_MTYPE && strcmp(buf.mtext, "ACK")==0){
            count++;
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < total_params; i++) {
        for (int j = 0; j < num_executables; j++) {
            ...
            msgsnd(worker_id, msg, sizeof(msg), msg.mtype); // --> maybe we are wrong DOUBLE CHECK WITH TA
            sent++;
        }
    }
    ...
    receive_ack_from_workers(msqid, num_workers){
        ...
        while(count < num_workers){
            ...
            if(msgrcv(msqid, &msg, sizeof(msg.text), msg.mtype, 0)==-1){
    ...
    send_synack_to_workers(msqid, num_workers){
        ...
        while(count < num_workers){
            struct msgbuf_t message;
            if(msgsnd(msqid, &msg, sizeof(msg.text), msg.mtype, 0)==-1){
                perror("msgrcv failed");
                exit(1);
            }
            count++;
